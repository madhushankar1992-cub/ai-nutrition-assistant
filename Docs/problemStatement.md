...
You are tasked with building an AI-powered chatbot that answers questions about food, nutrition and food safety using a Large Language Model (LLM). The first milestone uses the model's own knowledge without a retrieval layer. The prototype must expose unsupported claims, record failures and establish a stable foundation for adding cited evidence in Milestone 2.
