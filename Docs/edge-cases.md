...
| No rate limiting on the public endpoint (distinct from the Groq-facing limiter in `lib/rateLimiter.ts`) | The Groq-facing limiter protects against exceeding *Groq's* tier limits, but does nothing to stop one user from monopolizing the shared daily/per-minute budget — a script hitting `/api/chat` repeatedly would queue up (via the limiter) and starve every other concurrent user, or exhaust the 1,000 req/day or 200,000 tokens/day ceiling for everyone. Add per-IP or per-conversation rate limiting at the route level (e.g. Vercel Edge Config / Upstash) before/at public deployment (Phase 10). **GAP**. |
...
| Edge case | Risk if unhandled | Mitigation |
|---|---|---|
| Groq API is down, rate-limited, or times out | Every chat request fails; no graceful degradation | **Resolved**: `lib/groq.ts`'s `callOnce` retries `RateLimitError` (429, honoring `Retry-After`) and `InternalServerError`/`APIConnectionError` with backoff, separately from the schema-validation retry path. The route's existing catch-all still returns a clean fallback message if retries are exhausted. |
...
| Very long conversation history inflates token usage/cost silently over a long session | Cost and latency grow unbounded per conversation, and against Groq's 8,000 tokens/min limit an uncapped history could make a single call consume most or all of the per-minute budget | **Resolved**: `lib/groq.ts` caps history to the last `MAX_HISTORY_TURNS` (8) turns per call, independent of the rate limiter's own per-call token estimate. |
